...
Omega = 256 # podsnum
T = 2 # vectornum
sum_port = 256*2 # ports pair that an OXC provided
K = 128 # oxc nums in a vector
B = 100
...
with open('R_source.txt', 'r') as file:
    line = file.readline()
    R['sou_des'] = [[int(e)] for e in line.split()]

with open('R_desti.txt', 'r') as file:
    line = file.readline()
    mid = -1
    for e in line.split():
        mid += 1
        R['sou_des'][mid].append(int(e))

for _ in range(len(R['sou_des'])):
    R['route'].append([])

with open('R_route.txt', 'r') as file:
    lines = file.readlines()
    # 遍历每一行数据
    for line in lines:
        elements = line.split()
        key = int(elements[0])
        value = [int(e) for e in elements[1:]]
        R['route'][key].append(value)

# print(R)

S_numpy = np.zeros((sum_port, sum_port, K, T))
...
with open('S.txt', 'r') as file:
    ...
    for line in file:
        ...
        # S[k][l][j][i] = 1
        # S[l][k][j][i] = 1
        S_numpy[k,l,j,i] = 1 # 只填充了上三角矩阵，numpy数组

# 初始化一个空的嵌套列表数组
port_allocation = [[[[] for _ in range(2)] for _ in range(K)] for _ in range(T)]

# 将连接具体分配到端口上
for i in range(T):  # 有几个平面
    for j in range(K):  # 每个平面有几个oxc
        p = -1  # 指示oxc的端口序号
        for k in range(Omega):  # 指示当前pod序号
            for l in range(p + 1, p + G + 1):
                # 填充pod进去，标记该oxc端口与哪个pod相连
                port_allocation[i][j][0].append(k)
                # 标记当前端口的占用情况，0表示未占用
                port_allocation[i][j][1].append(0)
            p = p + G  # 更新

# print(port_allocation)

# 根据S来分配port_allocation的端口使用
indices = np.argwhere(S_numpy == 1)
for index in indices:
    port_allocation[index[3]][index[2]][1][index[0]] = 1
    port_allocation[index[3]][index[2]][1][index[1]] = 1

# 抽象成真正的物理拓扑，即S[pod, pod, K, T]，也是只填充了上三角
indices = np.argwhere(S_numpy == 1)
for index in indices:
    pod_s = port_allocation[index[3]][index[2]][0][index[0]]  # 源 POD
    pod_d = port_allocation[index[3]][index[2]][0][index[1]]  # 目的 POD
...
E_numpy = np.zeros((sum_port, sum_port, K, T))
...
with open('E.txt', 'r') as file:
    ...
    for line in file:
        ...
        # E[k][l][j][i] = 1
        # E[l][k][j][i] = 1
        E_numpy[k, l, j, i] = 1  # 只填充了上三角矩阵，numpy数组
...
for item in Distri:
    ...
    pod_s = port_allocation[k][l][0][s]
    pod_d = port_allocation[k][l][0][d]
...
for i in range(T):
    if STA[0, i] == 0 and STA[1, i] != 0:  # 该平面只需要增线
        ...
        for index in indices:
            # 检验端口是否空闲
            res = max(port_usage_numpy[i, index[2], index[0]], port_usage_numpy[i, index[2], index[1]]) # residual port number
            if res >= (-Delta[index[0],index[1],index[2],i]): # daibiaokeyizengjialianjie
                # 连接起来
...
if stages != -1:
    ...
    while len(remove) != 0: # 代表还有待拆的平面
        ...
        for item in remove: # 确定当前要重构的平面
            ...
            eta_basic = eta_den
            indices = np.argwhere(Delta[..., item] > 0) # chai
            for index in indices:  # 找到要拆除的连接(s)
                ...
                # pan duan de fang fa jiu shi kan ci lian ge pod zhi jian shi fou cun zai qi ta de ke ti dai lian jie
                    ...
                    if eta >= eta_th:  # 判断是否不小于阈值
                        ...
                        if min_value == 0: # Indicates that there is no traffic on this link
                            ...
                            for nape in range(len(Distri_pod['occupy'][seq])):
                                for nape_v2 in range(len(Distri_pod['occupy'][seq][nape]['Band_occupy'])): # each link
                                    if nape != inseq[0] or nape_v2 != min_index:
                                        if Distri_pod['occupy'][seq][nape]['Band_occupy'][nape_v2] != B:
                                            ...
                                            Alter_list[2].append(B - Distri_pod['occupy'][seq][nape]['Band_occupy'][nape_v2]) # residual capacity
                            ...
                            if total_band_kl >= min_value: # can be directly groommed
                                ...
                                for Elem in range(len(Record[0])):  # for each flow
                                    ...
                                    if Hops == 1:
                                        ...
                                        for a in range(2):
                                            judge = R['route'][Record[0][Elem][0]][Record[0][Elem][1]][a * 4:(a * 4 + 4)]
                                    ...
                                    while Record[1][Elem] != 0:
                                        ...
                                        if Times == 0:  # the first piece
                                            Distri_pod['occupy'][seq][Alter_list[0][o]]['Req'][Alter_list[1][o]].append(Record[0][Elem])
                                            # modify R
                                            R['route'][Record[0][Elem][0]][Record[0][Elem][1]][-1] = can_serve
                                            R['route'][Record[0][Elem][0]][Record[0][Elem][1]][loc] = Distri_pod['connections'][seq][Alter_list[0][o]][0]
                                            R['route'][Record[0][Elem][0]][Record[0][Elem][1]][loc + 1] = Distri_pod['connections'][seq][Alter_list[0][o]][1]

                                        else:
                                            # modify R
                                            rou_te = copy.deepcopy(R['route'][Record[0][Elem][0]][Record[0][Elem][1]])  # list
                                            rou_te[-1] = can_serve
                                            ...
                                            Distri_pod['occupy'][seq][Alter_list[0][o]]['Req'][Alter_list[1][o]].append([Record[0][Elem][0], len(R['route'][Record[0][Elem][0]]) - 1])

                                        Record[1][Elem] -= can_serve
                                        Alter_list[2][o] -= can_serve
                                        Distri_pod['occupy'][seq][Alter_list[0][o]]['Band_occupy'][Alter_list[1][o]] += can_serve  # Weighted equipartition
                                        Times += 1
                                        if Alter_list[2][o] == 0:
                                            o += 1
                                ...
                                copy_Distri = copy.deepcopy(Distri_pod) # 复制一份 但是由于目前还不确定是否能拆除当前连接，所以不能对Distri直接进行操作
                                ...
                                for o in range(len(Record[0])):
                                    ...
                                    if hop == 2:
                                        ...
                                        for a in range(hop):
                                            ...
                                            if judge != [item, j, k, l]:
                                                seq_1 = find_list_in_dict(copy_Distri, [judge[2], judge[3]], 'pod_pairs')
                                                nested_array_1 = np.array(copy_Distri['connections'][seq_1])  # transform to numpy array
                                                inseq_1 = np.where((nested_array_1 == [judge[0], judge[1]]).all(axis=1))[0]
                                                for b in range(len(copy_Distri['occupy'][seq_1][inseq_1[0]]['Band_occupy'])):
                                                    for c in range(len(copy_Distri['occupy'][seq_1][inseq_1[0]]['Req'][b])):
                                                        if copy_Distri['occupy'][seq_1][inseq_1[0]]['Req'][b][c] == Record[0][o]:
                                                            j_break = 1
                                                            del copy_Distri['occupy'][seq_1][inseq_1[0]]['Req'][b][c]
                                                            copy_Distri['occupy'][seq_1][inseq_1[0]]['Band_occupy'][b] -= Record[1][o]
                                                            break
                                ...
                                Logical_res = np.zeros((Omega, Omega)) # cunchuduiyingpod pairs deshengyukeyongrongliang
                                ...
                                for o in range(len(copy_Distri['pod_pairs'])):
                                    ...
                                    if mid != 0:
                                        Logical_res[copy_Distri['pod_pairs'][o][0], copy_Distri['pod_pairs'][o][1]] = mid
                                        Logical_res[copy_Distri['pod_pairs'][o][1], copy_Distri['pod_pairs'][o][0]] = mid
                                    ...
                                    if Logical_res[flow_sou, flow_des] != 0:
                                        ...
                                        if Logical_res[flow_sou, flow_des] >= Record[1][o]: # can serve
                                            for o_1 in range(len(copy_Distri['connections'][seq_1])):
                                                for o_2 in range(len(copy_Distri['occupy'][seq_1][o_1]['Band_occupy'])):
                                                    if copy_Distri['occupy'][seq_1][o_1]['Band_occupy'][o_2] != B:
                                                        FLAG += 1 # 当FLAG = 1时表示第一次分割请求，放入copy_R的原始位置
                                                        portion = min(B - copy_Distri['occupy'][seq_1][o_1]['Band_occupy'][o_2], Record[1][o])
                                                        if FLAG == 1:
                                                            copyR_List[o][Record[0][o][1]] = [copy_Distri['connections'][seq_1][o_1][0], copy_Distri['connections'][seq_1][o_1][1], flow_sou, flow_des, portion]
                                                            copy_Distri['occupy'][seq_1][o_1]['Band_occupy'][o_2] += portion
                                                            copy_Distri['occupy'][seq_1][o_1]['Req'][o_2].append(Record[0][o])
                                                        else:
                                                            copyR_List[o].append([copy_Distri['connections'][seq_1][o_1][0], copy_Distri['connections'][seq_1][o_1][1], flow_sou, flow_des, portion])
                                                            copy_Distri['occupy'][seq_1][o_1]['Band_occupy'][o_2] += portion
                                                            copy_Distri['occupy'][seq_1][o_1]['Req'][o_2].append([Record[0][o][0], len(copyR_List[o]) -1])
                                            ...
                                            for o_1 in range(len(copy_Distri['connections'][seq_1])):
                                                for o_2 in range(len(copy_Distri['occupy'][seq_1][o_1]['Band_occupy'])):
                                                    if copy_Distri['occupy'][seq_1][o_1]['Band_occupy'][o_2] != B:
                                                        FLAG += 1 # 当FLAG = 1时表示第一次分割请求，放入copy_R的原始位置
                                                        portion = B - copy_Distri['occupy'][seq_1][o_1]['Band_occupy'][o_2]
                                                        if FLAG == 1:
                                                            copyR_List[o][Record[0][o][1]] = [copy_Distri['connections'][seq_1][o_1][0], copy_Distri['connections'][seq_1][o_1][1], flow_sou, flow_des, portion]
                                                            copy_Distri['occupy'][seq_1][o_1]['Band_occupy'][o_2] = B
                                                            copy_Distri['occupy'][seq_1][o_1]['Req'][o_2].append(Record[0][o])
                                                        else:
                                                            copyR_List[o].append([copy_Distri['connections'][seq_1][o_1][0], copy_Distri['connections'][seq_1][o_1][1], flow_sou, flow_des, portion])
                                                            copy_Distri['occupy'][seq_1][o_1]['Band_occupy'][o_2] = B
                                    ...
                                    if Record[1][o] != 0: # find two hops
                                        for a in range(Omega):
                                            if a != flow_sou and a != flow_des:
                                                ...
                                                if Logical_res[pod_fir, pod_sec] != 0 and Logical_res[pod_thi, pod_fou] != 0:
                                                    # ava_capa = min(Logical_res[pod_fir, pod_sec], Logical_res[pod_thi, pod_fou])
                                                    seq_1 = find_list_in_dict(copy_Distri, [pod_fir, pod_sec],'pod_pairs')
                                                    seq_2 = find_list_in_dict(copy_Distri, [pod_thi, pod_fou],'pod_pairs')
                                                    ...
                                                    while Record[1][o] > 0:
                                                        while len(copy_Distri['occupy'][seq_1][o_1]['Band_occupy']) == 0:
                                                            o_1 += 1
                                                        ...
                                                        while len(copy_Distri['occupy'][seq_2][o_3]['Band_occupy']) == 0:
                                                            o_3 += 1
                                                        ...
                                                        if copy_Distri['occupy'][seq_1][o_1]['Band_occupy'][o_2] != B and copy_Distri['occupy'][seq_2][o_3]['Band_occupy'][o_4] != B:
                                                            FLAG += 1  # 当FLAG = 1时表示第一次分割请求，放入copy_R的原始位置
                                                            capa = min(B - copy_Distri['occupy'][seq_1][o_1]['Band_occupy'][o_2], B - copy_Distri['occupy'][seq_2][o_3]['Band_occupy'][o_4])
                                                            portion = min(capa, Record[1][o])

                                                            if FLAG == 1:
                                                                copyR_List[o][Record[0][o][1]] = [copy_Distri['connections'][seq_1][o_1][0], copy_Distri['connections'][seq_1][o_1][1], pod_fir, pod_sec,
                                                                                                  copy_Distri['connections'][seq_2][o_3][0], copy_Distri['connections'][seq_2][o_3][1], pod_thi, pod_fou, portion]
                                                                copy_Distri['occupy'][seq_1][o_1]['Band_occupy'][o_2] += portion
                                                                copy_Distri['occupy'][seq_1][o_1]['Req'][o_2].append(Record[0][o])
                                                                copy_Distri['occupy'][seq_2][o_3]['Band_occupy'][o_4] += portion
                                                                copy_Distri['occupy'][seq_2][o_3]['Req'][o_4].append(Record[0][o])
                                                            else:
                                                                copyR_List[o].append([copy_Distri['connections'][seq_1][o_1][0], copy_Distri['connections'][seq_1][o_1][1], pod_fir, pod_sec,
                                                                                                  copy_Distri['connections'][seq_2][o_3][0], copy_Distri['connections'][seq_2][o_3][1], pod_thi, pod_fou, portion])
                                                                copy_Distri['occupy'][seq_1][o_1]['Band_occupy'][o_2] += portion
                                                                copy_Distri['occupy'][seq_1][o_1]['Req'][o_2].append([Record[0][o][0], len(copyR_List[o]) - 1])
                                                                copy_Distri['occupy'][seq_2][o_3]['Band_occupy'][o_4] += portion
                                                                copy_Distri['occupy'][seq_2][o_3]['Req'][o_4].append([Record[0][o][0], len(copyR_List[o]) - 1])
                                                        ...
                                                        if copy_Distri['occupy'][seq_1][o_1]['Band_occupy'][o_2] == B:
                                                            o_2 += 1
                                                            if o_2 >= len(copy_Distri['occupy'][seq_1][o_1]['Band_occupy']):
                                                                o_1 += 1
                                                                o_2 = 0
                                                                if o_1 >= len(copy_Distri['connections'][seq_1]):
                                                                    break

                                                        if copy_Distri['occupy'][seq_2][o_3]['Band_occupy'][o_4] == B:
                                                            o_4 += 1
                                                            if o_4 >= len(copy_Distri['occupy'][seq_2][o_3]['Band_occupy']):
                                                                o_3 += 1
                                                                o_4 = 0
                                                                if o_3 >= len(copy_Distri['connections'][seq_2]):
                                                                    break
            ...
            for index in indices:
                ...
                for i in range(int(-Delta[k, l, j, item])):
                    ...
                    if res >= 1:
                        ...
                        else:  # xin zeng de pod pairs connections
