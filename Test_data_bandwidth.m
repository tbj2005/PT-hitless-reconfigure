...
% input.nodesnum = [4,7,8,16,32,64,128];
% input.groupnum = [2,2,2,2,4,4,4];
% % input.oxcports = [12,16,32,64,128,256];
% input.oxcports = [12,21,32,64,128,256,256];
% % input.oxcnum_agroup = [1,2,4,4,8,8];
% input.oxcnum_agroup = [1,2,2,2,2,4,4];
% input.connection_cap = [100, 100, 100, 100, 100, 100];
% input.physical_conn_oxc = input.oxcports./input.nodesnum;
% % input.max_num_requests = [4,24,100,400,1600,4000];
% input.max_num_requests = [4,24,100,400,800,1000];
% input.cap_ratio = [0.01,0.03,0.05,0.07,0.09];


input.nodesnum = [4,6,8,16,32,64,128,256];
input.groupnum = [2,2,2,2,4,4,4,32];
% input.oxcports = [12,16,32,64,128,256];
input.oxcports = [12,18,32,64,128,256,256,256];
% input.oxcnum_agroup = [1,2,4,4,8,8];
input.oxcnum_agroup = [1,1,2,2,2,4,4,16];
input.connection_cap = [100, 100, 100, 100, 100, 100, 100, 100];%%可能是cap打了，结果才不好，可以
input.physical_conn_oxc = input.oxcports./input.nodesnum;
% input.max_num_requests = [4,24,100,400,1600,4000];
input.max_num_requests = [4,24,100,400,800,1000];
input.cap_ratio = [0.01,0.03,0.05,0.07,0.09];
...
for i = 3
    ...
    for j = 10
        ...
        % flow_request = re_ger_flows(inputs,Logical_topo_init_cap);%已知一组流，增加流的数目
        ...
        RE{i}{j} = inputs.request;
        Logical_Topo{i}{j} = logical_topo;
        Tar_logical_topo{i}{j} = Logical_topo_desi;
        ...
        for m = 1:3
            ...
            % tic
            % if inputs.method == 1
            %     stage(j,m)  = reconfig_benchmark_fun(S,E,R,inputs,port_allocation);
            %     spendingtime = toc;
            % else
            %     % stage(i,j,m) = reconfig_progress_fun(S,E,R,inputs,port_allocation);% satge = 0代表可以直接加连接
            %     stage(j,m) = reconfig_progress_fun(S,E,R,inputs,port_allocation);% satge = 0代表可以直接加连接
            %     spendingtime1 = toc;
            % end
