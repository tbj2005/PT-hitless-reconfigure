...
%%% 该函数实现第一种方法
function [rest_add_delta_topo,update_logical_topo,update_delta_topo_del] = reconnect_links(inputs,logical_topo,Logical_topo_weight,update_delta_topo_add, update_logical_topo, update_delta_topo_del)
...
%% 一个平面的待增加链接的所有相关链接---决定要增加的链接
[sortValue, sortIndex] = sort(triu(update_delta_topo_add(:)),'descend');
[row, col] = ind2sub(size(update_delta_topo_add), sortIndex);
del_link_now_in_added = [row(1), col(1)];%或者说一次只处理一条链接 用for-loop
links_tobe_add_topo = zeros(inputs.nodes_num,inputs.nodes_num);
rest_del_update_logical_topo1 = update_logical_topo;
for t = 1:inputs.groupnum
    for k = 1:inputs.oxcnum_agroup
       
        rest_del_update_logical_topo1{t,k}(row(1),:) = 0;%k,t拓扑中删除连接后剩下的连接情况
        rest_del_update_logical_topo1{t,k}(:,row(1)) = 0;
        rest_del_update_logical_topo1{t,k}(:,col(1)) = 0;
        rest_del_update_logical_topo1{t,k}(col(1),:) = 0;
        % 要增加的链接不用删除
        rest_del_update_logical_topo1{t,k}(row(1),col(1)) = update_logical_topo{t,k}(row(1),col(1));
        rest_del_update_logical_topo1{t,k}(row(1),col(1)) = update_logical_topo{t,k}(row(1),col(1));

        del_update_logical_topo1{t,k} = update_logical_topo{t,k} - rest_del_update_logical_topo{t,k}; %k,t拓扑中删除的链接
    
        del_link_now_in_added = del_link_now_in_added + del_update_logical_topo1{t,k};
    end
end
% 对所有平面处理完删除的步骤后需要增加的链接
links_tobe_add_topo = links_tobe_add_topo - update_delta_topo_del;
links_tobe_add_topo(links_tobe_add_topo < 0) = 0;%%
...
for t = 1:inputs.groupnum
    for k = 1:inputs.oxcnum_agroup
        ...
        rest_del_update_logical_topo = rest_del_update_logical_topo1{t,k};
        del_update_logical_topo = del_update_logical_topo1{t,k};
