...
for t = 1:inputs.groupnum % vector 
    for k = 1:inputs.oxcnum_agroup
        ...
        for i = 1:size(subindex,1)
            ...
            update_logical_topo_cap{t,k}(subindex(i,1),subindex(i,2)) = can_add_conns_innodepair*inputs.connection_cap + update_logical_topo{t,k}(subindex(i,1),subindex(i,2)) - logical_topo_traffic{t,k}(subindex(i,1),subindex(i,2)) - logical_topo_traffic{t, k}(subindex(i,2),subindex(i,1));
